...
def get_image_page_url():
    html = requests.get(
        "https://wallhaven.cc/search?categories=110&purity=100&sorting=random&order=desc&ai_art_filter=1"
    ).content
...
def set_wallpaper(file_name):
    ...
    os.system(
        f'osascript -e \'tell application "System Events" to tell every desktop to set picture to "{file_location}"\''
    )
...
logging.info(f"Wallpaper set: '{page_url}'")
